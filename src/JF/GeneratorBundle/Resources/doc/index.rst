...
The ``JFGeneratorBundle`` extends the default Symfony2 command line
...
`Download`_ the bundle and put it under the ``JF\\Bundle\\`` namespace.
...
The ``JFGeneratorBundle`` comes with four new commands that can be run in
...
file structure in ``APP_PATH/Resources/JFGeneratorBundle/skeleton`` or
``BUNDLE_PATH/Resources/JFGeneratorBundle/skeleton`` if you want to extend
...
``APP_PATH/Resources/JFGeneratorBundle/skeleton``.
...
.. code-block: jinja
  ...
  {# in app/Resources/JFGeneratorBundle/skeleton/crud/actions/create.php.twig #}
